...
**蒙特卡洛树搜索** (Monte Carlo Tree Search, MCTS) 是一种在连续的或离散的动作空间上都可用的的策略，但在离散的动作空间上使用更容易，MCTS是用于求随性解闭环控制的复杂问题的更先进的工具。它也在AlphaGo的早期版本中承担很重要的作用。
...
1. 假设当前决策的根节点为$s_1$，使用某种$\text{TreePolicy}(s_1)$得到一个叶子节点$s_l$。
2. 使用某种随机策略（或其他）$\text{DefaultPolicy}(s_l)$评估该叶子节点。
...
- 否则就根据$\text{Score}(s_{t+1})$选取一个得分最大的儿子$s_{t+1}$，$\text{Score}(s_{t+1})=\frac{Q(s_{t+1})}{N(s_{t+1})}+2C\sqrt{\frac{2\ln N(s_t)}{N(s_{t+1})}}$，其中$Q(s_t)$为该节点为跟的子树的所有已经被展开过的节点的评分之和，$N(s_t)$为该节点为根的子树中已经被展开过的节点个数，因此第一项$\frac{Q(s_t)}{N(s_t)}$就是$s_t$的平均评分了，而第二项则是用来评估稀有性。
...
接下来将介绍一些最优控制算法，这些算法不像之前的使用微积分和导数的算法，因为有时导数是非常难得到的，比如在Atari游戏中并没有给出导数；不过如果可以写出系统的物理规律，那么可以通过自动微分等方式得到导数。

### 在最优控制算法中使用导数

在讨论关于导数之前，我们需要先假设系统中的转移动态是确定性的。在我们了解了确定性转移动态的思想之后，可以通过SSC算法让其对随机性转移动态也同样适用，即使是一个十分特殊的随机性分布。
...
在为非线性动态系统提供求解方法之前，我们先来研究一类简单的线性模型的控制问题，叫做LQR (Linear Quadratic Regulator, 线性二次型调节器) 控制器。这个问题与之前一样，试图最小化总开销，即
